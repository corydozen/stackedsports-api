class GetFavoritesJob < ApplicationJob
  queue_as :twitter
  ...
  def perform(_scheduled_time)
    ...
    athletes = Athlete.joins(:twitter_cache_date).where('twitter_cache_dates.favorites <= ?', Date.yesterday.end_of_day).or(Athlete.joins(:twitter_cache_date).where(twitter_cache_dates: { favorites: nil })).order('twitter_cache_dates.favorites')
    ...
    begin
      Parallel.each(-> { athletes_to_update.shift || Parallel::Stop }, in_threads: pool_size) do |athlete_id|
        ...
        coach.get_favorites(athlete)
        athlete.twitter_cache_date.favorites = Time.now
